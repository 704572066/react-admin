...
import { ListDataSetManagementDto, SaveUserManagementDto } from './dto';
...
export class DataSetManagementService {
  ...
  async getDataSetList(
    userInfo: ListDataSetManagementDto,
    ...
    // const lim = end-start;
    // const order = 1;
    const sort = 'updateTime';
    const username = '';
    const where = {
      username: username
    };

    const appsRaw = await this.prismaService.datasets.findMany({skip:start, take:end-start,orderBy:{
      [sort] :"asc" 
    }})
      // .skip(start)
      // .limit(end - start)
    ...
    const apps = appsRaw.map((app) => {
      ...
      return {
        ...
        updateTime: dayjs(app.updateTime).format('YYYY/MM/DD HH:mm')
        // password: ''
      };
    });

    const totalCount = await this.prismaService.datasets.count();
    ...
    //   distinct: true,
    // });
    return responseMessage({ list: apps, total: totalCount });
  }
