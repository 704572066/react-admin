...
import { Token } from '@/models/token.model'; // Channel 实体
...
import { TokenController } from './token.controller'; // RoleManagement Controller
import { TokenService } from './token.service'; // RoleManagement Service
@Module({
  // 将实体 导入到这个module中，以便你这个module中的其它provider使用
  imports: [
    SequelizeModule.forFeature([Token, Abilities]),
    OperationLogsModule,
