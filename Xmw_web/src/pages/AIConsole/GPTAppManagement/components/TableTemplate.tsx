...
import { delUser, getAppList, setUserStatus } from '@/services/ai-console/gpt-app-management' // 用户管理接口
...
const TableTemplate: FC = () => {
	...
	const { runAsync: fetchUserList } = useRequest(
		async (params) => formatResponse(await getAppList(params)), {
		manual: true,
	})
...
*/
	const columns: ProColumns<API.USERMANAGEMENT>[] = [
		...
		{
			title: formatMessage({ id: formatPerfix(ROUTES.GPTAPPMANAGEMENT, 'id') }),
			dataIndex: 'id',
			hideInSearch: true,
			ellipsis: true,
			align: 'center',
		...
		// },
		{
			title: formatMessage({ id: formatPerfix(ROUTES.GPTAPPMANAGEMENT, 'update_time') }),
