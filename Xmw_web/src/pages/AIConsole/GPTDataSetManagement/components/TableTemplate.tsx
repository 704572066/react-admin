...
import { delUser, getDataSetList, setUserStatus } from '@/services/ai-console/gpt-dataset-management' // 用户管理接口
...
const TableTemplate: FC = () => {
	...
	const { runAsync: fetchUserList } = useRequest(
		async (params) => formatResponse(await getDataSetList(params)), {
		manual: true,
	})
...
*/
	const columns: ProColumns<API.USERMANAGEMENT>[] = [
		...
		{
			title: formatMessage({ id: formatPerfix(ROUTES.GPTDATASETMANAGEMENT, 'id') }),
			dataIndex: 'id',
			hideInSearch: true,
			ellipsis: true,
			align: 'center',
		...
		{
			title: formatMessage({ id: formatPerfix(ROUTES.GPTDATASETMANAGEMENT, 'dataset_name') }),
			dataIndex: 'name',
			ellipsis: true,
			width: 100,
			align: 'center',
		...
		{
			title: formatMessage({ id: formatPerfix(ROUTES.GPTDATASETMANAGEMENT, 'vector_model') }),
		...
		{
			title: formatMessage({ id: formatPerfix(ROUTES.GPTDATASETMANAGEMENT, 'agent_model') }),
		...
		// },
		{
			title: formatMessage({ id: formatPerfix(ROUTES.GPTDATASETMANAGEMENT, 'update_time') }),
